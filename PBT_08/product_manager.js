...
// Dùng filter: Giữ lại những item thỏa mãn điều kiện
function getInStock(products) {
    return products.filter(p => p.stock > 0);
}

// 2. Lọc theo category VÀ khoảng giá
// Dùng filter kết hợp toán tử && (AND)
...
// Phải dùng [...products] để copy mảng trước khi sort, tránh làm thay đổi mảng gốc (mutate)
...
// Dùng reduce để gom nhóm dữ liệu vào một Object kết quả
function cheapestByCategory(products) {
    return products.reduce((acc, p) => {
        // Nếu danh mục này chưa có trong object, hoặc giá SP hiện tại rẻ hơn SP đã lưu
        if (!acc[p.category] || p.price < acc[p.category].price) {
            acc[p.category] = p; // Cập nhật SP rẻ nhất cho danh mục đó
        }
        return acc;
    }, {});
}
...
// Dùng reduce để cộng dồn một con số duy nhất
function totalInventoryValue(products) {
    return products.reduce((sum, p) => sum + (p.price * p.stock), 0);
}

// 6. Tạo mảng chỉ chứa { name, formattedPrice }
// Dùng map để biến đổi cấu trúc của từng phần tử
...
function averageRating(products) {
    if (products.length === 0) return 0; // Tránh lỗi chia cho 0
    const totalRating = products.reduce((sum, p) => sum + p.rating, 0);
    return parseFloat((totalRating / products.length).toFixed(1));
}

// 8. Tìm sản phẩm theo keyword (tìm trong name, case-insensitive)
// Chuyển cả keyword và tên SP về chữ thường để so sánh không phân biệt hoa/thường
...
console.log("\n=== TÌM KIẾM TỪ KHOÁ 'pro' ===");
console.log(formatProductList(searchProducts(products, "pro")));